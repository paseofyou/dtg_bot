...
## Stage-3 order-vs-Δt（TwiBot-22，已完成，commit 0054345 跑批）

分层抽样 10 万标注用户（split×label 保比例），6 变体 × 15 种子，
仅微观分支。逐种子结果：`experiments/results_t22_dt.csv`，
分析产物：`experiments/stage3_analysis/`（`scripts/analyze_stage3.py` 复算）。

关键结论（Holm 校正后，AUC 为准——T22 微观单支 AUC 仅 56~62、F1 退化不可信）：

1. **次序零效应外部复现**：keep vs shuffle ΔAUC +0.01 (p=0.972)；
   加时间通道后 keep_dt vs shuffle_dt 仍 n.s.（−0.71, p=0.185）。
   → "离散先后次序无信息"在两个数据集上独立成立，5.3 节闭环。
2. **连续时间通道有增量**：keep_dt−keep_base ΔAUC +1.62 (p_holm=3e-4)；
   bag_dt−bag_base ΔAUC +3.86 (p<1e-15)。
   → 无效的是"相对先后"而非"时间本身"——Δt/昼夜相位/星期携带信号。
3. **⚠️ 事件级注意力在 T22 上反向**：bag 的 AUC 显著优于 transformer
   （−1.98 与 −4.22，均校正显著），与 T20 的 +4.10 相反。
   → 注意力增益依赖事件特征信噪比，非无条件成立；论文已收窄表述。
