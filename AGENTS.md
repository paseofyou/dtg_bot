...
**获得支持：**
1. 微观视图（事件级注意力）叠加到图视图上，AUC 一致显著提升
   （global→micro+global +0.28 p=0.0044；macro→micro+macro +0.19 p=0.0085）。
2. 完整框架（concat 融合）vs BotRGCN：ΔF1 +0.49 (p=3e-4)、ΔAUC +0.49 (p<1e-4)。
3. 仅全局视图 vs BotRGCN：ΔF1 -0.03 (p=0.82) → **实现正确性检验通过**
   （全局分支确实等价于 BotRGCN 图通路）。

**未获得支持（负面结果，论文已如实报告）：**
4. **宏观快照演化无价值**：macro+global vs global ΔF1 +0.15 (p=0.34)、ΔAUC **-0.17**；
   GRU(K=8) vs last(终态快照) ΔAUC -0.10 (p=0.26)。
   推测主因：边形成时间不可观测，`max(c_u,c_v)<=t_k` 只反映注册次序而非真实演化。
5. **attn 融合显著劣于 concat**：ΔF1 -0.38 (p=0.019)、ΔAUC -0.21 (p=0.0041)；gate 亦优于 attn。
6. **InfoNCE 无增益**：AUC 随 λ 单调下降，λ=1.0 vs λ=0 ΔAUC -0.32 (p=0.0027)。

→ 统一解释：**视图判别力悬殊时（micro 的 F1 比 global 低 13 分），
强制对齐或加权求和会损害强视图。** 不要再往融合层加复杂度。

⚠️ **上述 p 值是 Welch 非配对近似**（只有 mean/std）。正式写入论文前必须用
`results.csv` 的逐种子值做**配对 t 检验**。另外 concat 作为主配置的选择
必须依据 `best_dev_f1` 而非 test F1，否则是测试集选择偏差。
