...
- **推文无时间戳，但列表顺序是真实的隐式时序信息。** 已在 TwiBot-22 上交叉验证 dump
  保持按 `created_at` 严格倒序（`2022-02-27` → `2022-02-24`），故该假设有据。
  使用时 reverse 成时间正序（旧→新）。
...
- 5 个种子 `42 123 456 789 2024`；同一组超参用于所有变体
- 按 val F1 选 checkpoint，test 只在最后评估一次
- 报 Accuracy / Precision / Recall / F1 / MCC 的 mean±std，并对关键对比做**配对 t 检验**
